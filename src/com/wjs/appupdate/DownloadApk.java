...
import com.wjs.utils.CommonUtil;
...
public class DownloadApk extends HttpServlet {
     ...
     final static Log log = LogFactory.getLog(DownloadApk. class);
     boolean debug=false;
     @Override
     ...
     protected void doPost(HttpServletRequest request, HttpServletResponse response) 
     {
           ...
           String name=DecoderEncoderUtils.decodeFilepath(undecode);
           if(debug)
           ...
           try 
           {
        	   if(debug)
                 log.info( "请求下载的连接地址为：" + request.getRequestURL() + "?" + request.getQueryString());
               	...
               	{
               		throw new IllegalArgumentException("请求下载的文件名参数为空");
               	}
           } 
           ...
           OutputStream out = null; // 缓冲
            ...
            { // 客户端请求的下载的文件块的开始字节
                response.setStatus(javax.servlet.http.HttpServletResponse. SC_PARTIAL_CONTENT);//设置返回码:206  告诉客户端支持断点续传
           ...
           response.setHeader( "Accept-Ranges", "bytes" );// 如果是第一次下,还没有断点续传,状态是默认的 200,无需显式设置;响应的格式是:HTTP/1.1 200 OK
            ...
            try 
            {
                response.addHeader( "Content-Disposition", "attachment; filename=\"" + downloadFile.getName() + "\"");//设置默认文件名字
